 ...
 * 
 * SIG Common Debug
 * Mostly non-minified version
 * 
 * This version of sig-common.js includes non-minified versions of the
 * JavaScript libraries that we've developed here at NIBR.
 * 
 * All third-party libraries (like jQuery) remain minified.
 * 
 */
;/*!
...
(function(A){A.fn.placehold=function(D){var D=D||"placeholder",C=A.fn.placehold.is_supported();function B(){for(i=0;i<arguments.length;i++){arguments[i].toggle();}}return C?this:this.each(function(){var F=A(this),E=F.attr("placeholder");if(E){if(!F.val()||F.val()==E){F.addClass(D).val(E);}if(F.attr("type")=="password"){var G=A("<input />",{"class":F.attr("class")+" "+D,value:E});G.bind("focus.placehold",function(){B(F,G);F.focus();});F.bind("blur.placehold",function(){if(!F.val()){B(F,G);}});F.hide().after(G);}F.bind({"focus.placehold":function(){if(F.val()==E){F.removeClass(D).val("");}},"blur.placehold":function(){if(!F.val()){F.addClass(D).val(E);}}});F.closest("form").bind("submit.placehold",function(){if(F.val()==E){F.val("");}return true;});}});};A.fn.placehold.is_supported=function(){return"placeholder" in document.createElement("input");};})(jQuery);// Get width of navigation bar
...
$(document).ready(function() {
    ...
    // Apply sig-button classes to all buttons within sig-content class
    $(".sig-content button, .sig-content input[type='submit']").not('.ui-widget').addClass("sig-button");
...
$(function(){
	...
	$.widget("ui.nebulaFeedback", {
		options: {
			assignee : null,
			project : null,
			issueType: 1,		// Automatically defaults the issuetype given a correct value
			inline: false ,		// Displays inline, rather than as dropdown
			hide: [],			// Hides the given fields. Must be in array form
			summary: null,		// Provides default data for the summary field
			issueTypes: [],	// Displays custom issue types. Must be in array form to work [{value: 81, label: "Example"}]
			notes: []			// Appends notes to the description upon submission. Must be in array form to work [{name: "Example",  value: 1}]
		},
	
		_create: function(option) {		   
				
			var feedBack = "<div class='sig-content feedback-form-dialog' style='display: block;'>\
				<p class='improve'>Please help us improve! Tell us what you were looking for or trying to do, and what you would like to see improved. Provide as much detail as possible, and feel free to include links to other sites that get the job done better. Thanks!</p>\
					<form class='feedback-form'>\
						<fieldset>\
							<label for='feedback-issuetype' class='above'>Issue Type</label>\
							<select class='feedback-issuetype'>\
								<option value='1'>Bug</option>\
								<option value='2'>New Feature</option>\
								<option value='12'>Other Feedback</option>\
							</select>\
						</fieldset>\
						<fieldset>\
							<label for='feedback-summary' class='above'>Summary</label>\
							<input type='text' class='feedback-summary' name='feedback-summary'>\
							<div class='form-error'></div>\
						</fieldset>\
						<fieldset>\
							<label for='feedback-description' class='above'>Description</label>\
							<textarea class='feedback-description' name='feedback-description'></textarea>\
							<div class='form-error'></div>\
						</fieldset>\
						<div class='sig-actions' id='feedback-form-actions'>\
							<img src='http://nebula.na.novartis.net/images/feedbackform/feedback-form-waiting.gif' id='feedback-form-loading'>\
							<button class='sig-button sig-button-icon sig-button-delete' id='feedback-form-cancel'>Cancel</button>\
							<button class='sig-button sig-button-icon sig-button-save' id='feedback-form-submit'>Submit</button>\
						</div>\
			...
			this.options.notes.push({name: "Screen Resolution",  value: screen.width + 'x' + screen.height});
				
			if(settings.collapseByDefault){
				$(".feedback-form-dialog").hide();
			}
			
			if (this.options.project == null || this.options.assignee == null || this.options.project == '' || this.options.assignee == '') {
				alert('Hi! It looks like you\'re viewing the SIG Dev Toolkit Template for the first time. '+
					'Before you use this template, you must edit the <head> section to configure the Feedback plugin. '+
					'Full instructions are on our SIG website. '+
					'If you\'re trying to download the Dev Toolkit Template, but you see this page, you\'re probably using Internet Explorer. Click your back button, then right-click the download link, and select "Save target as..."');
				return false;
			}
				
			$(this.element).bind('click', function() {
				if ($(".feedback-form-dialog").is(":visible")) {
					$(".feedback-form-dialog").slideUp();
				} else {
					$(".feedback-form-dialog").slideDown();
				}
			});
					
			// Cancel button
			$("#feedback-form-cancel").live('click', function(){
			...
			$("#feedback-form-submit").live('click', function() {
				...
				if (formValidated) {
					$("#feedback-form-actions").find("button").attr("disabled", "disabled");
					$("#feedback-form-loading").show();
					
					var description = $(".feedback-description").val();
					
					// Append additional notes if specified
					if (self.options.notes.length > 0) {
						
						description += "\n\n";
						
						// Notes option
						for (i = 0; i < self.options.notes.length; i++){
							description += self.options.notes[i].name + ": " + self.options.notes[i].value + "\n";
						};
					}
					
					$.ajax({
						type: "GET",
						url: settings.nebulaWebRoot + "feedback",
						dataType: "jsonp",
						data: {
								"project" : self.options.project,
								"assignee" : self.options.assignee,
								"issuetype" : $(".feedback-issuetype").val(),
								"summary" : $(".feedback-summary").val(),
						...
						success: function(resp) {
							if(resp.response.errors.length) {
								$.each(resp.response.errors, function(){
									if (this.field == 'jira') {
										$(".feedback-message").html("<strong>Sorry, your feedback could not be submitted.</strong> Please try again. (JIRA, our issue tracker, reported this error: '" + this.error + "')").slideDown();
									} else {
										var fieldID = '#feedback-'+this.field+'-error';
										$(fieldID).text(this.error);
									}
								});
							}
							else {
								$(".feedback-form")[0].reset();								
								$(".feedback-message").html('Thank you for your feedback. You will receive an e-mail shortly to track the status of your request. <a href="javascript:void(0);" onclick="$(\'#feedback-form-cancel\').click();">Close</a>').slideDown();
								$("#feedback-close-link").slideDown();
							}
							$("#feedback-form-loading").hide();
							$("#feedback-form-actions").find("button").removeAttr("disabled");
						}
					});
				}
			...
			if (this.options.inline) {
				...
				var example = $(".feedback-form").unwrap();
				$("#feedback-form-link").hide();
				$(this.element).append(example).addClass("sig-inline-feedback");
				$(".improve").hide();
			...
			// Issuetype option
			if (this.options.issueType != null){
				$(".feedback-issuetype").val(this.options.issueType);
			}
			
			// Hide option
			if ($.isArray(this.options.hide)){
				if ($.inArray("type", this.options.hide) > -1) {
					$(".feedback-issuetype").hide();
					$("label[for='feedback-issuetype']").hide();
				}
				if ($.inArray("summary", this.options.hide) > -1) {
					 $(".feedback-summary").hide();
					 $("label[for='feedback-summary']").hide();
				}
				if ($.inArray("description", this.options.hide) > -1) {
					$(".feedback-description").hide();
					$("label[for='feedback-description']").hide();
				}
			}
			
			// Summary option	
			$(".feedback-summary").val(this.options.summary);
			
			// Custom issueTypes option	
			if (this.options.issueTypes.length > 0) {
				// remove the default options
				$(".feedback-issuetype option").remove();
				for (i = 0; i < this.options.issueTypes.length; i++){ 
					$(".feedback-issuetype").append("<option value= " + this.options.issueTypes[i].value + ">" + this.options.issueTypes[i].label + "</option>");
				}
			}
				
			// Searches for a custom issueType. Can find the newly created issuetypes and set that as default value
			for (i = 0; i < this.options.issueTypes.length; i++) {
				if (this.options.issueTypes[i].value === this.options.issueType) {
					$(".feedback-issuetype").val(this.options.issueType);
				}	
			}
...
/*
 * A sidebar plugin created by Gary Katsevman.
 * ...and modified by Ari Entlich to be less hacky and work in more situations.
 * Also, setting the placement to right actually works now.
 *
 * On the item that is to be the sidebar, call this plugin. Give an object containing at least a `placement'
 *
 * $('#sidebar').sidebar({placement: 'left'})
 *
 * `placement' should be either `left' or `right'
 *
 *  Other options are:
 *  sidebarWidth - otherwise, uses the HTML's width
 *  images - an object with right and left and open and closed CSS class names, elaborated below.
 *  open - true to leave sidebar open, false to leave it closed.
 *
 *  images: {
 *      right: {
 *          open: "right-open",
 ...
 *  }
 *
 *  sidebar itself has 5 functions. It is initalized by passing an options object.
 *
 *      open - open the sidebar, if closed
 *      close - close the sidebar, if open
 *      disable - disable the click even on the separator so the sidebar could not be closed
 *      enable - reenable the closing of the sidebar via the separator click
 *      dispose - open the sidebar if closed, remove the separator
 *
 *  Use like so:
 *    $('#sidebar').sidebar('open');
 */
(function($) {
	function myfunc(settings, self, index) {
		var $separator,
			$sidebar,
			placement,
			sidebarWidth,
			toggle = false; // false: sidebar open, true: sidebar closed

		var clickEvent = function(event, toOpen) {
			$sidebar.show();
			$sidebar.animate(
				{width: $sidebar.width() == 1 ? sidebarWidth : 1},
				{duration: 500, complete: function() {
						if($sidebar.width() == 1)
							$sidebar.hide();

						$separator.toggleClass(images[placement].open);
						$separator.toggleClass(images[placement].closed);
					}});
		};
		...
		$sidebar = $(self).children();
		sidebarWidth = isUndefined(settings.sidebarWidth) ? $sidebar.width() : settings.sidebarWidth;
		placement = settings.placement.toLowerCase();
		var images = settings.images;

		if (isUndefined($sidebar) || isUndefined(placement) ||
			isUndefined(sidebarWidth) || isUndefined(images)) {
			throw new Error("Sorry, all options must be defined.");
		}

		$sidebar.width(sidebarWidth);

		$separator = $('<div />');
		$separator.addClass('sig-sidebar-separator');
		$separator.addClass(images[placement].open);

		var resizeSeparator = function() {
			var top = Math.max(0, $(window).scrollTop() - $(self).offset().top);
			var maxheight = $(self).parent().height() - top;

			$separator.css('top', top + 'px');
			$separator.height(Math.min(maxheight, $(window).height() + $(window).scrollTop() - $separator.offset().top));
		}
		$(window).resize(resizeSeparator);
		$(window).load(resizeSeparator);
		$(window).scroll(resizeSeparator);
		...
		$separator.bind('click', clickEvent);

		!settings.open && $separator.click();
		...
		$.data(self, 'clickEvent', clickEvent);
		$.data(self, 'separator', $separator);
		$.data(self, 'sidebar', $sidebar);
	}
		...
		init: function(options) {
			var settings = {
				...
				images: {
					right: {
						open: "sig-sidebar-separator-open-right",
						closed: "sig-sidebar-separator-closed-right"
					},
					left: {
						open: "sig-sidebar-separator-open-left",
						closed: "sig-sidebar-separator-closed-left"
					}
				},
				open: true
			};
			...
			return this.each(function(i, el) {
				myfunc(settings, this, i);
			});
		},

		open: function() {
			return this.each(function() {
				$.data(this,'clickEvent')(null, true)
			});
		},

		close: function() {
			return this.each(function() {
				$.data(this,'clickEvent')(null, false)
			});
		},

		disable: function() {
			return this.each(function() {
				$.data(this, 'separator').unbind('click', clickEvent)
			});
		},

		enable: function() {
			return this.each(function() {
				$.data(this, 'separator').bind('click', clickEvent)
			});
		},
		...
		dispose: function() {
			...
			return this.each(function() {
				$.data(this, 'sidebar').css('float','');
				$.data(this,'separator').remove();
			});
		}
	}
		...
		if (methods[method]) {
			...
			$.error('Method ' + method + ' does not exist on jQuery.lexicon');
		}    
	};
})(jQuery);
/**
...
$(document).ready(function() {
	...
	$("body").append('<div id="sig-person-preview" class="sig-person-preview sig-rounded-panel"><div class="photo"></div>'
	+ '<div class="spinner"></div>' + '<div class="person-details">' + '<a target="_blank" href="#"></a>' +
	'<div class="location"></div>' + 
	'<div class="email"><a></a></div>' + 
	'<div class="phone"></div>' + 
	'</div>' + '</div>');
	
	$(".sig-person").mouseover(function(e) {

		// need to have a rel set with 521
		if($(this).attr("rel") != '') {
			
			
			// first position the element
			var pos = $(this).offset();
			var vScrollPosition = $(document).scrollTop();
			var limitRight = $(window).width() - 300;
			var limitBottom = (vScrollPosition + $(window).height()) - 140;
			...
			// card width = 300px
			// if cursor is less than 300 px away from right window border, show card to left of cursor
			// if cursor is less than 200 px away from bottom window border, show card above cursor

			if(posHorizontal > limitRight) {
				posHorizontal = e.pageX - 300;
				offsetHorizontal = -10;
			}

			if(posVertical > limitBottom) {
				posVertical = posVertical - 140;
				offsetVertical = -5;
			}
			
			// reset the spinner
			$("#sig-person-preview .person-details").hide();
			$("#sig-person-preview img").attr('src', '');
			$("#sig-person-preview .spinner").show();

			// show it with the spinner
			$("#sig-person-preview").css('top', posVertical + offsetVertical).css('left', posHorizontal + offsetHorizontal).show();
			
			clearTimeout(sppTimeoutId);

			// now retrieve data
			$.ajax({
				url : 'http://uscalx1214.na.novartis.net/people/search.json',
				dataType : "jsonp",
				data : {
					term : $(this).attr("rel")
				},
				success : function(data) {
					...
					$("#sig-person-preview .person-details > a").html(person.firstname + ' ' + person.lastname).attr('href', 'http://mysite.na.novartis.net/profile/' + person.firstname + '.' + person.lastname);
					$("#sig-person-preview .location").html(person.site_name + ' &middot; ' + person.workplace);
					$("#sig-person-preview .email a").html(person.e_mail).attr('href', 'mailto:' + person.e_mail);
					$("#sig-person-preview .phone").html(person.phone);
					$("#sig-person-preview .photo").html('<img src="http://mysite.na.novartis.net/User%20Photos/Profile%20Pictures/'+getDomainFromSiteCode(person.site_code)+'_'+person.unique_id+'_MThumb.jpg" onerror="personImageError(this);" />');
				
					$("#sig-person-preview .spinner").hide();
					$("#sig-person-preview .person-details").show();
				}
			});
	...
	$(".sig-person").mouseout(function(e) {
		sppTimeoutId = window.setTimeout(function() {
			$("#sig-person-preview").hide();
		}, 300);
	});

	$("#sig-person-preview").mouseover(function(e) {
		window.clearTimeout(sppTimeoutId);
	});

	$("#sig-person-preview").mouseout(function(e) {
		sppTimeoutId = window.setTimeout(function() {
			$("#sig-person-preview").hide();
		}, 100);
	});
...
/*
 (function() {
 $.widget("ui.sigPersonPreview", {

 options : {
 service : 'http://uscalx1214.na.novartis.net/'
 },

 _create : function() {

 var self = this; // the plugin
 var element = this.element; // the element to which we apply the plugin
 ...
 return '<div class="sig-person-preview sig-rounded-panel">\
 <img />\
 <div class="sig-person-details">\
 <a class="person-name">TEST</a>\
 ...
 $("#debug").append("<pre>Showing preview." + '</pre>');

 $(".sig-person-preview").css('top', posVertical + offsetVertical).css('left', posHorizontal + offsetHorizontal).show();

 $("#debug").append("<pre>E OVER: Clearing timeout: " + element.timeout + '</pre>');

 window.clearTimeout(element.timeout);
 });

 element.mouseout(function(e) {
 element.timeout = window.setTimeout(function() { $(".sig-person-preview").fadeOut('fast'); }, 500);
 $("#debug").append("<pre>E OUT: Set timeout: " + element.timeout + '</pre>');
 });

 $(".sig-person-preview").mouseenter(function(e) {
 $("#debug").append("<pre>SPP ENTER: Clearing timeout: " + element.timeout + '</pre>');
 window.clearTimeout(element.timeout);
 })

 $(".sig-person-preview").mouseleave(function(e) {
 element.timeout = window.setTimeout(function() { $(".sig-person-preview").fadeOut('fast'); }, 100);
 $("#debug").append("<pre>SPP LEAVE: Set timeout: " + element.timeout + '</pre>');
 });
